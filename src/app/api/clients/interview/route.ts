...
export async function POST(request: Request) {
  ...
  const instruction = [
    ...
    "- BE BRIEF. A CA is doing this between calls. Aim to finish in three or four exchanges, not ten.",
    "- Ask for SEVERAL related facts in one short question. 'What does it do, where is it registered,",
    "  and roughly what turnover?' is one question, not three. Never ask for a single fact when two or",
    "  three neighbours can ride along.",
    "- One or two sentences per turn. No preamble, no acknowledgement of the last answer, no",
    "  restating what you already know, no bullet lists.",
    "- Ask about what is still missing, most important first. Never re-ask something already known,",
    "  and never re-ask something the CA already declined to answer.",
    "- Accept vague answers. 'Around 40 crore' is fine. 'Not sure' is a real answer — record nothing",
    "  for that attribute and move straight on without commenting on it.",
    "- When the important attributes are answered or the CA wants to stop, say the profile is ready",
    "  in one short line and set complete to true. Do not keep going for the sake of the remaining",
    "  attributes: a partial profile the CA finished is worth more than a full one they abandoned.",
    "",
